...
// GET de todas as recs com objecto ZONA e SURTO associado
...
router.get('/', async function (req,res) {
    try {
        ...
        if(recs) {
            ...
            for (const rec of recs) {
                const zona_mae = await Zona.findById(rec.cod_zonageo).exec();
                const surto_pai = await Surtos.findById(rec.cod_surto).populate('cod_virus').exec();
...
router.get('/:cod_recomendacao', async function(req, res) {
    try{
        ...
        const surto_pai = await Surtos.findById(recomendacao.cod_surto).populate("cod_virus").exec();
